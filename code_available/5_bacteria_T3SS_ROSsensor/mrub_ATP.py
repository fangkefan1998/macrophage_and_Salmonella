# 本代码用于分析ATP-A81D-hslu-mrubs使用过氧化氢处理之后的小黑点氧化水平
import cv2
...
# 大致思路：读取514的图像，阈值分割，判定小黑点，并且将foci和even的mask分别储存在两个不同的list，然后用这些list中的mask分别与405和488通道相乘，得到每一个菌的405和488平均亮度，然后减去背景之后做除法

# 首先读取514的图像进行阈值分割以及小黑点的判定
img = cv2.imread("D:\\Foci_Identify\\mRubs\\561\\1344-hslu-mrubs-A81D-macrophage-08.czi - 1344-hslu-mrubs-A81D-macrophage-08.czi #1 - C=3.tif")
...
imraw = matplotlib.image.imread('D:\\Foci_Identify\\mRubs\\561\\1344-hslu-mrubs-A81D-macrophage-08.czi - 1344-hslu-mrubs-A81D-macrophage-08.czi #1 - C=3.tif')
...
for k in range(0, storage.__len__()):
    ...
    if s == 1:
        path = "D:\\Foci_Identify\\mRubs\\Data\\foci\\" + str(k) + "foci.png"
        ...
        path = "D:\\Foci_Identify\\mRubs\\Data\\even\\" + str(k) + "even.png"
        single_normal = normalization.normalize(single)
        im = Image.fromarray(single_normal * 255.0)
        im.convert('L').save(path)
...
# 读入405和488通道的图像，准备计算每个细菌的亮度，需要读取原始的16bit图像
img405 = matplotlib.image.imread("D:\\Foci_Identify\\mRubs\\405\\1344-hslu-mrubs-A81D-macrophage-08.czi - 1344-hslu-mrubs-A81D-macrophage-08.czi #1 - C=1.tif")
img488 = matplotlib.image.imread("D:\\Foci_Identify\\mRubs\\488\\1344-hslu-mrubs-A81D-macrophage-08.czi - 1344-hslu-mrubs-A81D-macrophage-08.czi #1 - C=2.tif")

# 计算小黑点的单个细胞的405平均荧光强度,建立foci ratio以及even ratio对计算的比值进行储存，405背景2820，488背景1124!!!!!
foci_ratio = []
for n in range(0, foci_list.__len__()):
    FociMask = foci_list[n]
    single405 = img405*FociMask
    single488 = img488*FociMask
    calculate405 = single405[single405 > 0]
    intensity405 = np.sum(calculate405)/(calculate405.__len__())

    calculate488 = single488[single488 > 0]
    intensity488 = np.sum(calculate488)/(calculate488.__len__())
    ratio = (intensity405 - 1678)/(intensity488 - 1401)
    foci_ratio.append(ratio)
# 两个背景都要记得修改！！！！！！！！！！！！！！！
even_ratio = []
for n in range(0, even_list.__len__()):
    EvenMask = even_list[n]
    single405 = img405 * EvenMask
    single488 = img488 * EvenMask
    calculate405 = single405[single405 > 0]
    intensity405 = np.sum(calculate405) / (calculate405.__len__())
    ...
    intensity488 = np.sum(calculate488) / (calculate488.__len__())
    ratio = (intensity405 - 1678) / (intensity488 - 1401)
    even_ratio.append(ratio)


# 对于计算的foci和even的结果将其保存为csv进行输出
foci_output = pd.DataFrame(data=foci_ratio)
foci_output.to_csv("D:\\Foci_Identify\\mRubs\\Data\\Foci_intensity.csv", encoding="gbk")

even_output = pd.DataFrame(data=even_ratio)
even_output.to_csv("D:\\Foci_Identify\\mRubs\\Data\\Even_intensity.csv", encoding="gbk")
