# 本代码用于分析ROSsensor-Hslu-mEcitrine使用过氧化氢处理之后的小黑点氧化水平
import cv2
...
img = cv2.imread("D:\\Foci_Identify\\ROSsensor\\Data\\ROS-mscarlet\\ROS-mscarlet-488-02.tif")
...
imraw = matplotlib.image.imread('D:\\Foci_Identify\\ROSsensor\\Data\\ROS-mscarlet\\ROS-mscarlet-561-02.tif')
...
for k in range(0, storage.__len__()):
    ...
    if s == 1:
        path = "D:\\Foci_Identify\\ROSsensor\\foci\\" + str(k) + "foci.png"
        ...
        path = "D:\\Foci_Identify\\ROSsensor\\even\\" + str(k) + "even.png"
        single_normal = normalization.normalize(single)
        im = Image.fromarray(single_normal * 255.0)
        im.convert('L').save(path)
...
img405 = matplotlib.image.imread("D:\\Foci_Identify\\ROSsensor\\Data\\ROS-mscarlet\\ROS-mscarlet-405-02.tif")
img488 = matplotlib.image.imread("D:\\Foci_Identify\\ROSsensor\\Data\\ROS-mscarlet\\ROS-mscarlet-488-02.tif")
...
for n in range(0, foci_list.__len__()):
    ...
    ratio = (intensity405 - 2833)/(intensity488 - 1127)
...
for n in range(0, even_list.__len__()):
    ...
    calculate488 = single488[single488 > 0]
    intensity488 = np.sum(calculate488) / (calculate488.__len__())
    ratio = (intensity405 - 4809) / (intensity488 - 878)
    even_ratio.append(ratio)


# 对于计算的foci和even的结果将其保存为csv进行输出
foci_output = pd.DataFrame(data=foci_ratio)
foci_output.to_csv("D:\\Foci_Identify\\ROSsensor\\Data\\ROS-mscarlet\\Foci_intensity.csv", encoding="gbk")

even_output = pd.DataFrame(data=even_ratio)
even_output.to_csv("D:\\Foci_Identify\\ROSsensor\\Data\\ROS-mscarlet\\Even_intensity.csv", encoding="gbk")
